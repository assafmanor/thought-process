...
class Thought:
    ...
    def serialize(self):
        header = struct.pack(HEADER_FORMAT, self.user_id, int(dt.datetime.timestamp(self.timestamp)), len(self.thought))
        return header + self.thought.encode()


    def deserialize(data):
        user_id, timestamp, thoughtlen = struct.unpack(HEADER_FORMAT, data[:20])
