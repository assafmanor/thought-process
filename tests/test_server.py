...
from thoughtprocess import run_server


_SERVER_ADDRESS = '127.0.0.1', 5000
_SERVER_PATH = pathlib.Path(__file__).absolute().parent.parent / 'thoughtprocess' / 'server.py'
...
def _upload_thought(user_id, timestamp, thought):
    ...
    with socket.socket() as connection:
        time.sleep(0.2) # Wait for server to start listening.
